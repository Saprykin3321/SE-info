...
| Основной поток   | 1. Покупатель переходит в корзину и нажимает «Оформить заказ». <br> 2. Система запрашивает контактные данные (ФИО, телефон, email) и адрес доставки.<br> 3. Покупатель заполняет форму и выбирает способ оплаты. <br> 4. Система проверяет корректность данных и наличие товаров на складе. <br> 5. Система создаёт заказ со статусом «Новый», резервирует товары и отображает номер заказа. <br> 6. Покупатель перенаправляется на страницу оплаты (если выбран онлайн-способ). |
| Расширение       | ...                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |

#### 2 Просмотр списка заказов (менеджер)
...
| Основной поток   | 1. Менеджер переходит в раздел «Заказы». <br> 2. Система отображает таблицу заказов с фильтрами по статусу, дате, покупателю. <br> 3. Менеджер может выбрать заказ для просмотра деталей или изменения статуса. |
| Расширение       | ...                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| Предусловие      | Менеджер или складкской оператор авторизованы в системе                                                                                                                                                                                                                |
| Триггер          | Пришло уведомление о новом заказе                                                                                                                                                                                                                                      |
| Основной поток   | 1. Пользователь открывает детали заказа. <br> 2. Выбирает новый статус из выпадающего списка (например, «Передан в доставку»). <br> 3. Система проверяет допустимость перехода (например, из «Оплачен» можно перейти в «Передан в доставку», но не из «Отменён»). <br> 4. Статус изменяется, в историю заказа добавляется запись.|
| Расширение       | ...

### 2.2 Описание макетов интерфейса

Макеты приведены в [Figma](https://www.figma.com/).

#### 1 Экран списка заказов

![Изображение](interface/interface-1.jpg)

Описание экранной формы:

| № | Параметр  | Источник данных | Описание поведения                                    | 
|:--|:----------|:----------------|:------------------------------------------------------|
| 1 | Заголовок | -               | Отображается всегда <br> 1. Текст: Заказы покупателей |                                                                                                                                                                                                                                                                                                                  
| 2 | ...       | ...             | ...                                                   |
...
#### 3.1.1 Таблица `customers` (покупатели)
...
#### 3.1.3 Таблица `order_items` (позиции заказа)
